'use client'
import {
  UserGroupIcon,
  HomeIcon,
  DocumentDuplicateIcon,
  ShoppingCartIcon,
  TagIcon,
  MapPinIcon,
  ShoppingBagIcon
} from '@heroicons/react/24/outline';
import Link from 'next/link';
import { usePathname } from 'next/navigation';

// Map of links to display in the side navigation.
// Depending on the size of the application, this would be stored in a database.
const links = [
  { name: 'Reservas', href: '/Reservas', icon: HomeIcon },
  { name: 'Inventario', href: '/Inventario', icon: ShoppingCartIcon },
  { name: 'Notas', href: '/Notas', icon: TagIcon },
  { name: 'Lugares', href: '/Lugares', icon: MapPinIcon },
  { name: 'Clientes', href: '/Clientes', icon: ShoppingBagIcon },
  {name: 'Guia para la app',href: '/Guia',icon: DocumentDuplicateIcon,},
  { name: 'Soporte', href: '/Soporte', icon: UserGroupIcon },
];

export default function NavLinks() {
  const pathname = usePathname();
  return (
    <>
      {links.map((link) => {
        const LinkIcon = link.icon;
        return (
          <Link
            key={link.name}
            href={link.href}
            className={`flex h-[48px] grow items-center justify-center gap-2 
            rounded-md bg-gray-50 p-3 text-sm font-medium hover:bg-sky-100
             hover:text-blue-600 md:flex-none md:justify-start md:p-2 md:px-3 
             ${pathname === link.href ? "bg-blue-200 text-dark rounded-xl" : ""}
             `}>
            <LinkIcon className="w-6" />
            <p className="hidden md:block">{link.name}</p>
          </Link>
        );
      })}
    </>
  );
}
